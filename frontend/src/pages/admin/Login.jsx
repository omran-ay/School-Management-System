...
function Login() {
  ...
  const handleSubmitData = async (e) => {
    ...
    try {
      ...
      console.log("result.data.data.success", result?.data?.status);
      ...
      console.log("error.respons?.data", error.response?.data?.message);
    }
  };
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <container className="w-full max-w-md bg-white p-6 rounded-lg shadow-lg">
        ...
        <Link to="/forgotPassword" className="hover:text-red-600 underline">
          Forgot Password?
        </Link>
      </container>
    </div>
